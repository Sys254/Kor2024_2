...
BoardService4
    ...
    - 기존코드 : 고정 길이인 Board[] boardList = new Board[100];
    - 수정조건 :
        고정길이가 아닌 가변길이 형식으로 수정하여 100개 아닌
        무한개 저장 가능한 가변길이 배열 만들기

     * 이유 : 가변 길이의 여러개 데이터를 관리할때는
...
public class BoardService4 { /* 실행 클래스 */
    public static void main(String[] args) {
        ...
        //-가변길이 배열 만들기 예제
        Board[] boardList = null;// - Board 객체 100개를 저장할 수 있는 배열 선언
        // - 현재 게시물 수를 저장하는 변수
        int count = 0;
        while (true) {
            ...
            if (choose == 1) {
                ...
                Board board = new Board();// -2. 입력받은 데이터로 게시물 객체 생성
                ...
                // 기존 배열내 게시물들을 데이터를 새로운 배열에 이동하기 / 배열 복사
                if(count != 1){ // 만약에 기존 배열내 게시물이 존재하면
                    for(int index = 0; index <= boardList.length-1; index++ ){
                        newBoardList[index] = boardList[index]; // 기존 배열내 게시물들을 새로운 배열에 대입 / 카피+복사
                    }

                }

                // 새로운 배열내 마지막 인덱스(배열명.length-1)에 입력받은 게시물 객체 등록, add()
